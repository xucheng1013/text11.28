        ...
        function(obj) {
            ...
            $.each(obj, function(i, val) {
                if(val.item_code.substr(2, 4) == '0000'){
                    areaCont += '<li onClick="selectPNew(\''+val.item_code+'\',\''+val.item_name+'\');">' + val.item_name + '</li>';
                }
            });
...
function selectPNew(pcode,pname) {
	var areaCont = "";
	areaList.html("");
	$.each(cityJson, function(i, val) {
        if(val.item_code.substr(0, 2) == pcode.substr(0, 2) && val.item_code.substr(2, 4) != '0000' && val.item_code.substr(4, 2) == '00'){
            areaCont += '<li onClick="selectCNew(\''+val.item_code+'\',\''+val.item_name+'\',\''+pcode+'\',\''+pname+'\');">' + val.item_name + '</li>';
        }
    });
	...
	$("#areaBox").scrollTop(0);
	expressArea = pname +"&nbsp;";
	$("#backUp").attr("onClick", "intProvinceNew();").show();
}

function selectCNew(ccode,cname,pcode,pname) {
	var areaCont = "";
	var cityVal = ccode;
	$.each(cityJson, function(i, val) {
        if(cityVal=='110100' || cityVal=="120100" || cityVal=="310100" || cityVal=="500100"){
            if(val.item_code.substr(0, 3) == cityVal.substr(0, 3) && val.item_code.substr(4, 2) != '00'){
                areaCont += '<li onClick="selectDNew(\''+val.item_code+'\',\''+val.item_name+'\');">' + val.item_name + '</li>';
            }
        }else{
	...
	$("#backUp").attr("onClick", "selectPNew(" + pcode+','+pname + ");");
